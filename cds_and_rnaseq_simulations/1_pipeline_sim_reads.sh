...
# dependencies
# these need to be installed and accessible in your $PATH
# * gnu-parallel
# * samtools
# * bwa
# * featureCounts
...
# make bed and gff files from fasta
ls *fasta | parallel --plus bash fasta_to_bed.sh {} > {.}.bed
ls *fasta | parallel --plus bash fasta_to_gff.sh {} > {.}.gff
